...
$TestName = 'ProjectPLoginLobbyTest'
...
try {
    ...
    $gauntletRunLog = Join-Path $resolvedLogRoot "Gauntlet_LoginLobby_$RunId.out.log"
    Write-Step 'Run Gauntlet Login Lobby Test'
    ...
    if ($exitCode -ne 0) {
        throw "Gauntlet Login Lobby test failed. See log: $gauntletRunLog"
    }

    Write-Step 'Completed'
    Write-Host 'Gauntlet Login Lobby test completed.'
